...
- 前台首页视觉基准已切到“景德镇手工陶瓷 / 侘寂东方极简 / 跨境高信任电商”：白底、大量留白、浅陶土米色区块、哑光墨黑主按钮、暗朱红折扣/强调标签、真实产品摄影、统一 Header。
...
- 背景：全站纯白为主，信任区、订阅区、公告条使用浅陶土米色 `#EAE5DD`，不使用大面积彩色渐变、装饰球、复杂纹理。
- 分隔：细线边框，颜色使用 `var(--line)`。
- 主色：哑光墨黑 `#222222` 用于导航、主按钮、标题文字；暗朱红 `#912E2E` 只用于折扣、Bestseller/New 标签、CTA hover、优惠文字。
- 装饰：少即是多，优先使用茶具、陶瓷、匠人、礼盒等真实摄影、留白和排版层级，不用无意义图形装饰。
- 卡片：只用于商品、表单分组、重复列表、弹窗、工具面板，不把整页 section 做成大卡套小卡。
- 圆角：普通卡片不超过 8px；主按钮可用胶囊圆角；工具型输入框使用 6px 左右。

## 字体

- 标题：Inter/系统无衬线粗体，使用 `.premium-display`；不要再使用旧的 Serif 画册标题作为首页默认基准。
- 正文：Inter/系统无衬线常规字重。
...
- 前台 buyer-facing 页面以白、墨黑、陶土米色为主，暗朱红只做少量强调，避免一页一个配色。
...
- 前台页面使用“景德镇陶瓷侘寂极简电商”风格，不出现突兀颜色、花哨弹窗、浓烈促销色或营销式大装饰。
- 后台页面保持运营工具风格，信息密度合适。
- 手机、平板、桌面无横向溢出。
- 买家侧页面中英文都可读。
- 图片不变形，详情媒体不阻塞滑动。
