...
3. Admin order operations now include order detail, payment confirm/cancel in mock mode, exception tag, and latest failure reason at first usable level. Manual compensation trigger and audit trail are still required before production handoff.
4. Admin inventory operations now include reservation ledger, confirm/cancel status lookup, human-readable stock fields, and manual release at first usable level. Inventory action audit, stock adjustment, stocktaking, and safety-stock alerts are still required before production handoff.
...
| --- | --- | --- |
| Business loop | Inventory action audit, stock adjustment/stocktaking, safe order compensation action, and complete cross-border product operations | Operators cannot manage the full order and inventory lifecycle |
...
- Admin order operations: order list, exception red tag, latest failure reason, order detail, order line snapshots, and Mock payment confirm/cancel are implemented at first usable level through `order-service`, `admin-gateway`, and admin UI. Manual compensation trigger and audit logging are still pending.
- Inventory operations: human-readable fields, reservation ledger, confirm/cancel status lookup, and manual release for reserved reservations are implemented at first usable level through `inventory-service`, `admin-gateway`, and admin UI. Inventory action audit, stock adjustment, stocktaking, and safety-stock alerts are still pending.
