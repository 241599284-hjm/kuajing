...
- Publish action runs required-field validation and marks the import task as published.
...
The current recovery block does not implement real crawling, real AI copywriting, real AI image generation, media localization, asynchronous queue workers, or the final catalog publish adapter.
...
8. Publish to `catalog-service` through a stable adapter, not by directly writing catalog tables from the import service.
