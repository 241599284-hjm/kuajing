...
export function ServerStatusDashboard() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-xl font-semibold">服务器状态仪表盘</h1><p className="mt-1 text-xs text-[var(--muted-foreground)]">{message} · 每 30 秒自动刷新</p></div><Button size="sm" variant="outline" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? "animate-spin" : ""} size={14}/>立即刷新</Button></div>
    {data ? <>
