...
5. 建立统一业务错误码和错误文案表，替换随意 `message`。
...
8. 继续恢复误删模块：运维 SSL/CDN/统计后台、商品导入 AI 工作流、统一错误码包和前台合规/支付结果页面；物流轨迹服务、`/track-order` 页面、评价服务和后台审核已有第一版，但真实聚合物流 Provider、评论图片上传联动和一单一评订单校验仍需接入。
