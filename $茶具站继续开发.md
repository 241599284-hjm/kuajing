...
- 后台“订单管理”已新增订单详情区，可从订单列表查看订单详情，并能触发 Mock 支付确认/取消；操作失败时显示失败状态，不伪造成功。异常订单人工补偿入口和审计日志仍未完成。
- 后台“库存管理”已新增库存预留流水区，可读取 reservation 状态并对 reserved 预留执行人工释放；释放失败时显性提示，不伪造成功。库存操作审计、盘点/调整和安全库存告警仍未完成。
...
1. 后台订单管理补异常订单手工补偿入口和操作审计日志。
2. 后台库存管理补操作审计、盘点/调整、安全库存告警。
3. 在 Docker/RDS 可用环境验证 worker-service 对 PostgreSQL `compensation_tasks` 的实际重试和 DLQ 入库。
4. 修复/启动本地 PostgreSQL Docker 依赖，跑 catalog migration 并验证 `GET /catalog/ready`、`GET /catalog/storefront`、`PUT /catalog/categories`、`PUT /catalog/regions`、`PUT /catalog/products`，同时验证 Redis 缓存命中和写入后失效。
5. 后台商品管理继续补 image alt、多图排序和批量校验；HS Code、材质、原产地、容量、包装尺寸、重量、海关说明第一版已落地。
6. 继续完善 `media-service`：catalog 绑定失败补偿、响应式多尺寸变体、GIF 转视频、视频 poster/时长提取、上传审计日志。
7. 建立统一业务错误码和错误文案表，替换随意 `message`。
8. 继续补后台列表/详情/弹窗/分页 primitives，给后续订单、库存、Provider、DLQ 页面复用。
9. 按 `docs/production-gap-register.md` 继续推进 P0：真实 PostgreSQL 故障演练、Redis 缓存维度验证、统一错误码、全后台审计日志、对象存储绑定补偿。
