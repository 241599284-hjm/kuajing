...
9. 补齐自动化邮件真实闭环：注册验证时效策略后台化第一版已完成；继续补注册成功、订单确认、发货、退款、取消、评价邀请的真实业务触发、模板发布审计、失败记录治理；所有发送点统一走 `notification-service`。
