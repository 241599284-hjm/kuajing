...
2. 继续完善 `media-service`：catalog 绑定事务、响应式多尺寸变体、GIF 转视频、视频 poster/时长提取、后台多图排序/alt 字段落地；上传/删除审计和对象删除补偿入口已有第一版。
3. 继续扩大统一业务错误码接入范围：`inventory-service`、`order-service`、`catalog-service`、`review-service` 核心异常已接入第一批，下一步把 media/logistics/notification 的核心异常逐步替换为标准 `code/message/details`。
...
6. 继续恢复和打磨误删模块：商品导入 AI 工作流已有第一版；物流轨迹服务、`/track-order` 页面、评价服务、后台审核、统一错误码包、合规页、支付结果页、Cookie 提醒、国际电话字段、市场偏好组件、茶壶等待层、订单收货地址快照和运维 SSL/CDN/统计后台已有第一版；评价提交的订单校验已接入 `order-service`，但真实聚合物流 Provider、评论图片上传联动、站点配置动态化、后台 Cookie/市场配置、真实云 API 执行器、商品导入真实抓取/AI/媒体/队列/发布适配器和全服务错误码替换仍需接入。
