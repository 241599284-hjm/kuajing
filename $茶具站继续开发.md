...
- 测试服务器已部署 release `20260620165150-638053f23a6c`：首页 10 模块正式发布入库，三件 Catalog 商品、订阅运营和管理员登录均通过公网验证。浏览器认证统一使用同源 `/auth`，无需开放 auth-service 端口。
- 首页重要操作规则：保存草稿不影响买家页；发布和删除必须二次确认；所有资源写入继续拒绝 `http://`；后台“测试连通性”必须真实穿透 `admin-gateway -> store-service -> PostgreSQL`。
- PayPal Sandbox/Live/Webhook 配置已完成本地持久化：两套环境在 `order_db` 隔离存储，Secret 使用 AES-256-GCM 且永不回显，更新和测试写审计；Sandbox 允许 owner/finance 更新，Live 仅 owner。payment-service 创建订单、退款和 Webhook 验签按 `PAYPAL_ENVIRONMENT` 动态读取保存配置；OAuth 和 Webhook ID 查询测试均为真实 PayPal 请求。有效凭据成功分支和测试服务器部署仍未验证。
