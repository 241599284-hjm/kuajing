...
- 2026-06-17：服务器 smoke 发现 `ops-service` 运维接口运行时 repository 注入为空导致 `/ops/settings` 和 `/ops/actions/*` 返回 500；已补 fallback repository，避免运维配置页在 DI 失败时整体不可用。后续仍需补服务级集成测试覆盖 ops settings/action 路由。
