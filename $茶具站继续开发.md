...
- 2026-06-17：Catalog PostgreSQL/Redis 端到端缓存演练已在测试服务器通过。新增 `scripts/run-server-catalog-cache-drill.ps1`，通过 SSH 预热 `api-gateway /catalog/storefront` 和 `/catalog/products`，验证 Redis 中 `local:store:<storeId>:catalog:*` 五类 key 存在；随后通过 `admin-gateway PUT /catalog/categories` 临时更新 `teapot` 分类，验证 `categories` key 被刷新、`product-summaries/storefront-products/storefront` 依赖 key 被删除、`api-gateway /catalog/storefront` 返回新值，最后恢复原分类名并确认临时值消失；脚本复跑通过，correlation ID `cache-drill-1781696609`。当前真实演练覆盖分类写入，地域/商品写入仍需扩展同类演练。
...
1. 扩展 `scripts/run-server-catalog-cache-drill.ps1`，把地域写入和商品写入也纳入真实 PostgreSQL/Redis 演练，覆盖 `regions`、`productSummaries`、`storefrontProducts`、`storefront` 全部依赖路径。
2. 后台商品管理继续补 image alt、多图排序和批量校验；HS Code、材质、原产地、容量、包装尺寸、重量、海关说明第一版已落地。
3. 继续完善 `media-service`：catalog 绑定事务、响应式多尺寸变体、GIF 转视频、视频 poster/时长提取、后台多图排序/alt 字段落地；上传/删除审计和对象删除补偿入口已有第一版。
4. 继续扩大统一业务错误码接入范围：`@commerce/error-codes` 包和网关归一化已有第一版，下一步把 catalog/media/order/inventory/review/logistics/notification 的核心异常逐步替换为标准 `code/message/details`。
5. 继续补后台列表/详情/弹窗/分页 primitives，给后续订单、库存、Provider、DLQ 页面复用。
6. 按 `docs/production-gap-register.md` 继续推进 P0：Catalog 地域/商品真实 Redis 演练、统一错误码、catalog/media/product 写审计扩面、对象存储绑定补偿、真实支付/物流/邮件 Provider 故障演练。
7. 继续恢复和打磨误删模块：商品导入 AI 工作流已有第一版；物流轨迹服务、`/track-order` 页面、评价服务、后台审核、统一错误码包、合规页、支付结果页、Cookie 提醒、国际电话字段、市场偏好组件、茶壶等待层、订单收货地址快照和运维 SSL/CDN/统计后台已有第一版；评价提交的订单校验已接入 `order-service`，但真实聚合物流 Provider、评论图片上传联动、站点配置动态化、后台 Cookie/市场配置、真实云 API 执行器、商品导入真实抓取/AI/媒体/队列/发布适配器和全服务错误码替换仍需接入。
8. 补齐自动化邮件真实闭环：注册验证时效策略后台化和邮件 API 账号池持久化第一版已完成；继续补真实 Tencent SES API 发送、注册成功/订单确认/发货/退款/取消/评价邀请的真实业务触发、模板发布审计、失败记录治理；所有发送点统一走 `notification-service`。
9. 补齐评价收集真实闭环：一单一评订单校验第一版已接入；继续补订单签收延时邀评、评价图片走 media-service、审核通过同步前台、驳回留存、审核审计和邮件通知。
10. 继续补齐企业资质生产闭环：真实附件照片上传/绑定、每日自动到期扫描、提醒邮件真实 Provider 验证、资质操作权限和审计硬化。
