...
2. 配置有效 PayPal Sandbox Client ID/Secret/Webhook ID，完成真实创建/审批/capture、签名 webhook、重复事件、金额/币种不一致和真实退款 E2E。
3. 继续把旧业务面板内部控件迁移到新 shadcn 层，补订单筛选/分页/物流、PayPal 配置持久化和全局搜索真实接口。
4. 按 `docs/production-gap-register.md` 推进剩余 P0，不重复实现已完成项。
5. 补评价图片、签收后邀评、审核审计；补企业资质附件、每日到期扫描和真实提醒。
6. 自动化媒体定期演练，并为生产 HTTPS CDN/对象存储公开读取配置正式域名。
