...
- 第一台测试服务器信息已开始纳入部署流程：Ubuntu Server 24.04 LTS、公网 IP `170.106.67.141`、默认用户 `ubuntu`；已在本机生成新 SSH 密钥 `C:\Users\xx\.ssh\hlandteaware_tencent_rsa`，需把对应公钥绑定到腾讯云实例或追加到 `/home/ubuntu/.ssh/authorized_keys` 后才能 SSH 部署。
- 新增 Ubuntu 单机部署脚本：`infra/deploy/ubuntu-bootstrap.sh` 和 `scripts/deploy-to-server.ps1`。脚本会安装 Docker/Compose、放通基础端口、用 `git archive` 上传当前提交、在服务器启动 `docker compose --profile app up -d --build`，用于先上线一版测试站，再继续补 P0 缺口。
