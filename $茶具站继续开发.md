...
3. 服务端统一业务错误码第一轮已覆盖 `inventory-service`、`order-service`、`catalog-service`、`review-service`、`logistics-service`、`notification-service`、`media-service`；前台交易关键页和后台订单/库存/物流/评价/商品媒体页已接入共享中英文文案，下一步继续迁移邮箱设置、DLQ、运维配置、商品导入、分类/地域等配置型页面的局部错误字符串。
