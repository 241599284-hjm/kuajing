...
- 测试服务器已部署 release `20260621023000-f02b13cf9edf`，上一版为 `20260621014400-4149a0cd37d0`：订单服务端筛选/分页、首页 10 模块、三件 Catalog 商品、订阅运营、管理员登录和 PayPal 配置后台均通过相应验证。浏览器认证统一使用同源 `/auth`，无需开放 auth-service 端口。
...
- 订单后台已完成服务端筛选/分页并部署：支持订单/支付状态、日期、金额、订单号、买家邮箱和 PayPal 交易标识搜索，默认 20 条、最大 100 条，并提供总数、总页数、前后翻页和手动跳页。订单列表已按统一规范改为固定单行摘要和右侧详情按钮，完整详情使用独立接口与全局单实例弹窗；弹窗版本待本轮发布。
