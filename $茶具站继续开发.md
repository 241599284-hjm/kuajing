...
- `ops-service` 已恢复第一版运维配置底座：支持 EdgeOne 免费 SSL 证书、Let's Encrypt、Cloudflare/EdgeOne 免费 CDN、GA4/GSC 统计配置、企业资质资料读取/保存、运维动作记录和审计事件；PostgreSQL 可用时持久化，数据库不可用时明确内存降级。
- 后台已恢复“运维配置”入口，可维护 EdgeOne 免费证书/Let's Encrypt 证书来源、域名、强制 HTTPS、自动续期、Cloudflare/EdgeOne CDN Provider、CDN 不缓存白名单、真实 IP、基础防护、GA4/GSC/电商事件开关、企业资质资料和提醒邮箱，并可记录 EdgeOne 免费证书申请、手动续签、HTTP 检测、CDN 刷新、统计检测、资质到期扫描等操作。真实 EdgeOne、Let's Encrypt、Cloudflare、GA4/GSC API 执行器仍未接入，不伪造云端成功。
