...
1. 在腾讯云测试服务器执行 Catalog PostgreSQL/Redis 端到端缓存演练：`GET /catalog/storefront` 预热缓存，`PUT /catalog/categories|regions|products` 写入后台数据，确认相关 Redis key 被删除并且前台接口读取最新值。
...
6. 按 `docs/production-gap-register.md` 继续推进 P0：Catalog 真实 Redis 演练、统一错误码、catalog/media/product 写审计扩面、对象存储绑定补偿、真实支付/物流/邮件 Provider 故障演练。
