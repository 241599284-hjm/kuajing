...
- 自动化邮件新增硬需求已开始落地：注册验证、注册成功、订单确认、发货通知、退款通知、订单取消 6 类场景已进入 `notification-service` 默认模板；后台事务邮件模板页已支持英文 HTML 预览、保存/启停和发送日志查看；订单创建已触发 `order_confirmation`，物流发货通知入口已改用标准 `shipping_notice` 模板。真实 Tencent SES 账号池、退款/取消真实业务触发、模板发布审计、验证链接策略后台化仍需继续。
...
- `ops-service` 已恢复第一版运维配置底座：支持 SSL、Cloudflare 免费 CDN、GA4/GSC 统计配置读取/保存、运维动作记录和审计事件；PostgreSQL 可用时持久化，数据库不可用时明确内存降级。
- 后台已恢复“运维配置”入口，可维护 Let's Encrypt 域名/强制 HTTPS/自动续期、CDN 不缓存白名单/真实 IP/基础防护、GA4/GSC/电商事件开关，并可记录手动续签、HTTP 检测、CDN 刷新、统计检测等操作。真实 Let's Encrypt、Cloudflare、GA4/GSC API 执行器仍未接入，不伪造云端成功。
- 企业资质维护新增硬需求已登记：后台需维护营业执照、进出口权、海关备案、企业对公账户等字段、附件照片和截止日期；到期前 1 个月复用 `notification-service` 给管理员发提醒。当前字段和提醒任务尚未实现。
...
8. 继续恢复和打磨误删模块：商品导入 AI 工作流已有第一版；物流轨迹服务、`/track-order` 页面、评价服务、后台审核、统一错误码包、合规页、支付结果页、Cookie 提醒、国际电话字段、市场偏好组件、茶壶等待层和运维 SSL/CDN/统计后台已有第一版，但真实聚合物流 Provider、评论图片上传联动、一单一评订单校验、站点配置动态化、后台 Cookie/市场配置、真实云 API 执行器、商品导入真实抓取/AI/媒体/队列/发布适配器和全服务错误码替换仍需接入。
9. 补齐自动化邮件真实闭环：注册验证时效策略后台化、注册成功、订单确认、发货、退款、取消、评价邀请模板预览/发布/启停/日志/失败记录；所有发送点统一走 `notification-service`。
10. 补齐评价收集真实闭环：订单签收延时邀评、一单一评订单校验、评价图片走 media-service、审核通过同步前台、驳回留存、审核审计和邮件通知。
11. 补齐企业资质资料维护：营业执照、进出口权、海关备案、企业对公账户、附件照片、截止日期和到期前 1 个月邮件提醒。
