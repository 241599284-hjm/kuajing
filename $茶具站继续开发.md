...
- 自动化邮件新增硬需求已开始落地：注册验证、注册成功、订单确认、发货通知、退款通知、订单取消 6 类场景已进入 `notification-service` 默认模板；后台事务邮件模板页已支持英文 HTML 预览、保存/启停和发送日志查看；订单创建已触发 `order_confirmation`，物流发货通知入口已改用标准 `shipping_notice` 模板，企业资质到期提醒已新增 `company_credential_expiry` 模板。真实 Tencent SES 账号池、退款/取消真实业务触发、模板发布审计、验证链接策略后台化仍需继续。
