...
const defaultTemplates: EmailTemplateRecord[] = [
  {
    ...
    htmlZh: "<p>您好 {{name}}，</p><p>请点击下面的链接完成邮箱验证：</p><p><a href=\"{{verificationUrl}}\">验证邮箱</a></p><p>验证码：{{verificationCode}}</p>",
    htmlEn: "<p>Hello {{name}},</p><p>Please verify your email address:</p><p><a href=\"{{verificationUrl}}\">Verify email</a></p><p>Code: {{verificationCode}}</p>",
    textZh: "您好 {{name}}，请打开链接完成邮箱验证：{{verificationUrl}}，验证码：{{verificationCode}}",
    textEn: "Hello {{name}}, please verify your email address: {{verificationUrl}}. Code: {{verificationCode}}",
    enabled: true,
