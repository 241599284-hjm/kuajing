...
export function createDefaultHomepageLayout(): HomepageLayout {
  const modules: HomepageModule[] = [
    ...
    { id: "material-details", type: "materialDetails", enabled: true, sortOrder: 60, content: { eyebrow: text("Material Details", "材料细节"), title: text("The beauty is in the surface", "美，藏在器物表面"), body: text("Natural ash, mineral-rich clay, and restrained glazes make every piece subtly individual.", "天然草木灰、富含矿物的陶土与克制釉色，让每件作品保留独特肌理。"), imageUrl: "/assets/porcelain-tea-set-photo.jpg" } },
    ...
    { id: "newsletter", type: "newsletter", enabled: true, sortOrder: 80, content: { eyebrow: text("Private Notes", "私享手记"), title: text("New work, quietly announced", "新品，安静地抵达"), body: text("Receive first access to limited pieces and stories from the studio.", "优先收到限量作品与工作室故事。"), ctaLabel: text("Subscribe", "订阅"), imageUrl: "/static/ferncliff-artisan.png" } },
    ...
    .map((module, index) => {
      ...
      return {
        ...
        content: {
          ...
          items: module.content.items?.map((item) => ({ ...item, imageUrl: normalizeUrl(item.imageUrl) }))
        }
      };
    });
